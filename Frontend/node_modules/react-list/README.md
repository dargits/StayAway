...
A function to override the default translate: transform positioning behavior. For example,
you could set `getListStyle={({ x, y }) => ({ position: 'relative', left: x,
top: y })}` to use relative positioning.
